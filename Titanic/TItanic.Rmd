...
We have learnt some of the techniques below in the last 6 months or so and I will use them to predict the outcome of **_Titanic Survival Challenge_**.  
...
### i'm changing something here!!!!! delete later
...
## 2.1 Reviewing downloaded data from Kaggle
 ...
 * The Age feature has  a lot of NAs, in this case 177.
 ...
 * Fare has some outliers where you can see max value is a lot higher than mean and median
 
```{r}
summary(download$Fare)
```

 * There are two entries missing embarkation points
```{r}
...
Both Survived and Pclass have now been converted to factors as can be seen below
```{r}
...
I will replace all these NAs with mean values for each field.
...
```
  ...
  * Confirmed no more NAs in Age
```{r}
summary(download$Age)
```

  * Confirmed no more NAs in Fare
```{r}
summary(download$Fare)
```


## 2.4 Replacing missing embarkation points
...
## 2.4 Splitting the data to train and test
...
Generally during any major disasters, women and kids will be given priority whilst the men will do the hardwork to help others escape the disasters. 
...
Initially, I was going to remove this feature from my model until I stumbled on this sample data below.

If you look at the results in details you see a trend based on the same sex, pclass and family size. People embarked from Southampton had higher chance to die than people embarked from other ports. And this data is based on passenger class-3 and make me wondering why? The only thing I can think of is probably to do with the cabin location. Unfortunately, most of the rows have blank information in this field and I can't really use this field for my model. But, I believe people embarked from different ports for class-3 ended up in similar cabin locations in Titanic and unfortunately people from Southampton placed in locations where they were the hardest hit by iceberg. As a result of this, I will include embarked to be part of the features to determine if someone survived. 
...
If you look at the plot below for passenger class-3, you will see the bar chart showing ~50% mark for Cherbourgh and Queenstown whilst Southampton was a lot worse than the other two.
...
I will create further two groups to cater between 200 to 300 and beyond 300 as per below code.
```{r}
...
```{r}
fit.rf <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="rf", metric=metric, trControl=control)
fit.rpart <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="rpart", metric=metric, trControl=control)
fit.glm <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="glm", metric=metric, trControl=control)
fit.knn <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="knn", metric=metric, trControl=control)
fit.xgbTree <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="xgbTree", metric=metric, trControl=control)
fit.SVM <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="svmRadial", metric=metric,trControl=control)
...
fit.rf <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="rf")
fit.rpart <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="rpart")
fit.glm <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="glm")
fit.knn <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="knn")
fit.xgbTree <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="xgbTree")
fit.SVM <- train(Survived ~ Sex + Pclass + AgeGrp + FamilySize + Fare + Embarked, data=train, method="svmRadial")
...
# 4.1 Random Forest with default setting

Using the default setting in Caret for Random Forest, we can see the details of the best model. 
The default parameter is mtry using values of 2,4 and 7. Mtry is basically random predictors chosen by the model when building various different trees. It is very interesting to see the best accuracy was achieved using 2 mtry (2 predictors) only.

I can probably fine tune this since I have 5 features as my predictors so this can be changed instead of using default values. 

```{r}
print(fit.xgbTree)
```

The plot below shows the three default values in Random Forest and 2 predictos give the maximum accuracy around 83.7%.
```{r}
ggplot(fit.rf_final, highlight = TRUE)
```

Another function in caret called final model will show the error rate of 15.83%. This is basically related to our accurarcy of ~84% and hence the error rate is the residual around 16%. 
```{r}
fit.rf$finalModel
```
...
I am going to change the parameter using below code to run from 2 to 5 features to find the best mtry and to improve accuracy from the the current rate of 83.7%. In addition, I am also going to increase my cross validation to 15 and repeating it 3 times to see if my result will improve.
...
As you can see from the above results, my accuracy improved to 83.9% and mtry changed from 2 to 3. 
...
The accuracy is around 87% with balanced accuracy around 85% so this is a good result. I am not expecting the model to predict the outcome 100% since there were a lot of factors not captured in the dataset and I think to get around 80% accuracy is pretty good result already.
...
## 4.3 Predicting the test dataset

I am going to apply my prediction againt the test dataset which the model has not seen. 
...
The final out come against test dataset showing accuracy of 81.3% with balanced accuracy about 78% so it is quite balanced in predicting 0 and 1. 
